...
class TutorUserService:
    ...
    async def add(cls, session: AsyncSession, data: TutorStudentCreateSchema) -> TutorStudentSchema:
        try:
            instance: TutorStudent = await TutorStudentRepository.add(session=session, data=data.model_dump())
        except IntegrityError:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
        return TutorStudentSchema.model_validate(instance, from_attributes=True)
